...
class StepFunctionWorkflowStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        finalize_decision = self.create_lambda("FinalizeDecision", "finalize_decision.py")
        ...
        definition = sfn.Chain \
            ...
            .next(
                is_high_risk
                .when(sfn.Condition.string_equals("$.risk", "high"), wait_for_approval.next(finalize_task))
                .otherwise(finalize_task)
            )

        # State machine
        state_machine = sfn.StateMachine(
            self, "LoanApplicationStateMachine",
